...
public class FreeWriteController {
	...
	public ModelAndView form(@RequestParam("mem_id") String mem_id,
										   @RequestParam("writer") String writer) {//메서드명은 임의로 작성
		 System.out.println("다시 처음부터 값을 입력을 받기 위해서(초기화) form()호출됨.");
		//return "freeBoardWrite";//return "이동할페이지명"//definition name과 동일
		 ModelAndView mav=new ModelAndView("freeBoardWrite");
		 mav.addObject("mem_id",mem_id);//회원id
		 mav.addObject("writer",writer);//닉네임
		 return mav;
	}
	/*
	@RequestMapping(value="/board/freeBoardWrite.do",method=RequestMethod.GET)
	public ModelAndView form() {//메서드명은 임의로 작성
		 System.out.println("다시 처음부터 값을 입력을 받기 위해서(초기화) form()호출됨.");
		//return "freeBoardWrite";//return "이동할페이지명"//definition name과 동일
		 ModelAndView mav=new ModelAndView("freeBoardWrite");
	...
	public String submit(@ModelAttribute("FVO") FreeBoardVO com,
									@RequestParam(value="mem_id",defaultValue="hey") String mem_id,
						            @RequestParam(value="writer",defaultValue="hee") String writer){
			...
			if(log.isDebugEnabled()) {//로그객체가 디버깅모드상태인지 아닌지를 체크
				...
				log.debug("/board/freeBoardWrite.do의 mem_id=>"+mem_id);
				log.debug("/board/freeBoardWrite.do의 writer=>"+writer);
