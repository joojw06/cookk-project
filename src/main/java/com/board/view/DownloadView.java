...
public class DownloadView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		File file=(File)model.get("downloadFile"); //Object->(File)로 형변환
		System.out.println("file=>"+file);
		//2.다운받을 파일의 타입,크기를 지정
		response.setContentType(getContentType()); //"application/download"써도됨
		...
		if(ie) {
			fileName=URLEncoder.encode(file.getName(),"utf-8");//utf-8로 설정->한글이 안깨짐
		}else { //영문인경우
			fileName=new String(file.getName().getBytes("utf-8"),"iso8859-1");//영문O
		}
		//대화상자에서 원하는 위치를 다운로드 대화상자에서 지정
		//Content-Disposition=>다운로드받는 위치 지정,"attachment;filename=다운로드 받을 파일명
		//exe,bat=>실행파일(이진파일)도 다운받으려면=>Content-Transfer-Encoding을 binary로 지정
		...
		try {
			...
			FileCopyUtils.copy(fis, out);//copy(다운로드 받는 쪽 입력객체명, 서버의 출력객체명(접속객체))
		}catch(IOException e) {
			e.printStackTrace();
		}finally {//예외처리와 상관없이 항상 처리해야할 구문(메모리해제)
			if(fis!=null)
				try {
					fis.close();
				}catch(IOException e) {}
